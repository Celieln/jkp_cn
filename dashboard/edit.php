...
$语句 = $连接->prepare("SELECT * FROM jkp WHERE id=?");
$语句->bind_param("i", $编号);
$语句->execute();
$结果 = $语句->get_result();
$数据 = $结果->fetch_assoc();
...
<div class="topbar">
    ...
    <span class="brand-mobile">编辑数据</span>
    <span></span>
</div>
...
<div class="card-header">
    <h4 class="fw-bold mb-0" style="color:#fff"><i class="bi bi-pencil-square me-2"></i>编辑失业保障数据</h4>
</div>
<div class="card-body">
<form action="update.php" method="POST">
<input type="hidden" name="id" value="<?= $数据['id']; ?>">

<div class="section-title">裁员数据</div>
<div class="row g-3 mb-4">
<div class="col-md-6">
<label>登记编号</label>
<input type="text" name="nomor_register" class="form-control" value="<?= htmlspecialchars($数据['nomor_register']); ?>" required>
</div>
<div class="col-md-6">
<label>公司名称</label>
<input type="text" name="nama_perusahaan" class="form-control" value="<?= htmlspecialchars($数据['nama_perusahaan']); ?>" required>
</div>
<div class="col-md-6">
<label>员工姓名</label>
<input type="text" name="nama_pekerja" class="form-control" value="<?= htmlspecialchars($数据['nama_pekerja']); ?>" required>
</div>
<div class="col-md-3">
<label>身份证号</label>
<input type="text" name="nik" class="form-control" maxlength="16" value="<?= htmlspecialchars($数据['nik']); ?>" required>
</div>
<div class="col-md-3">
<label>裁员日期</label>
<input type="date" name="tanggal_phk" class="form-control" value="<?= $数据['tanggal_phk']; ?>" required>
</div>
<div class="col-12">
<label>裁员原因</label>
<textarea name="alasan_phk" class="form-control" required><?= htmlspecialchars($数据['alasan_phk']); ?></textarea>
</div>
</div>
...
<div class="col-md-6">
<label>解雇信函编号</label>
<input type="text" name="nomor_surat_lphk" class="form-control" value="<?= htmlspecialchars($数据['nomor_surat_lphk']); ?>" required>
</div>
<div class="col-md-6">
<label>解雇信函日期</label>
<input type="date" name="tanggal_surat_lphk" class="form-control" value="<?= $数据['tanggal_surat_lphk']; ?>" required>
</div>
</div>

<div class="d-flex gap-2 pt-2">
<button class="btn-glass primary px-4"><i class="bi bi-check-lg"></i> 更新</button>
